...
void  c_hacapk_adot_body_lfmtx_hyp_calc2
(double *zau, stc_HACApK_leafmtxp *st_leafmtxp, double *zu, double *zbu,
 double *time_batch, double *time_set, double *time_copy, int nd, int *ltmp) {
  static int count = 0;
#pragma omp parallel
  {
    register int ip,il,it;
...
#pragma omp parallel for reduction(+:zrnorm)
  ...
  if (mpinr == 0) {
    if(*lb==0)printf( "\n ** BICG (c version, OMP) **\n" );
    else printf( "\n ** BICG (c version, OMP(LoadBalance)) **\n" );
    printf( "\nOriginal relative residual norm = %.2e/%.2e = %.2e\n",zrnorm,bnorm,zrnorm/bnorm );
...
void  c_hacapk_adot_body_lfmtx_hyp_calc_mkl
(double *zau, stc_HACApK_leafmtxp *st_leafmtxp, double *zu, double *zbu,
 double *time_batch, double *time_set, double *time_copy, int nd) {
#pragma omp parallel
  {
...
void c_hacapk_bicgstab_cax_lfmtx_hyp_mkl_
(stc_HACApK_leafmtxp *st_leafmtxp, stc_HACApK_lcontrol *st_ctl,
 double *u, double *b, double*param, int *nd, int *nstp, int *lrtrn) {
  // local constants
  int converged = 0;
...
#pragma omp parallel for
  ...
  tic = MPI_Wtime();
  for(i=0;i<(*nd);i++)zshdw[i]=0.0;
  c_hacapk_adot_body_lfmtx_hyp_calc_mkl(zshdw,st_leafmtxp,u,wws, &time_batch,&time_set,&time_copy,*nd);
  /*
  {
    FILE *F;
    F=fopen("omp1.dat","w");
...
#pragma omp parallel for reduction(+:zrnorm)
  ...
  //return;
  if (mpinr == 0) {
	printf( "\n ** BICG (c version, OMP + MKL-seq) **\n" );
	printf( "\nOriginal relative residual norm = %.2e/%.2e = %.2e\n",zrnorm,bnorm,zrnorm/bnorm );
	printf( "c_HACApK_bicgstab_cax_lfmtx_hyp_mkl start\n" );
  }
  for ( step=1; step<=mstep; step++ ) {
    //for(step=1; step<=1; step++){
    if(zrnorm/bnorm < eps){converged++; break;}
